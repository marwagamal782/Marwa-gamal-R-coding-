...
G4_howell[is.na(G4_howell$weight) & G4_howell$Sex=="Male"  , 'weight']=m_meant
...
G4_howell$height<-gsub(" cm" ,"" , G4_howell$height)
G4_howell$height<-as.numeric(G4_howell$height)
...
#8)Display The ratio of normal and abnormal person
...
#9)Subset only males who have weight greater than  200
...
fig1<-ggplot(howell , aes(x=height  , y= weight))
...
fig2<-ggplot(howell , aes(weight , height))
